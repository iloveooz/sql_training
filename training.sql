...
-- 60. Заполнение таблицы с внешними ключами
/*
...
-- 62. Соединение INNER JOIN
/*
Операция соединения JOIN предназначена для обеспечения выборки данных из двух таблиц и 
включения этих данных в один результирующий набор. При необходимости соединения не двух, 
а нескольких таблиц, операция соединения применяется несколько раз (последовательно).
...
-- 63. Внешнее соединение LEFT и RIGHT OUTER JOIN
/*
Вывести все жанры, книг которых нет на складе.
*/
select g.name_genre as name_genre 
from genre g left outer join book b
...
-- 64. Перекрестное соединение CROSS JOIN
/*
...
order by c.name_city asc, 3 desc;
/*
Query result:
+-----------------+------------------+------------+
| name_city       | name_author      | Дата       |
+-----------------+------------------+------------+
| Владивосток     | Есенин С.А.      | 2020-12-12 |
| Владивосток     | Булгаков М.А.    | 2020-09-16 |
| Владивосток     | Пастернак Б.Л.   | 2020-08-11 |
| Владивосток     | Лермонтов М.Ю.   | 2020-05-16 |
| Владивосток     | Достоевский Ф.М. | 2020-02-02 |
| Москва          | Достоевский Ф.М. | 2020-11-20 |
| Москва          | Пастернак Б.Л.   | 2020-11-17 |
| Москва          | Есенин С.А.      | 2020-07-28 |
| Москва          | Булгаков М.А.    | 2020-07-15 |
| Москва          | Лермонтов М.Ю.   | 2020-01-07 |
| Санкт-Петербург | Достоевский Ф.М. | 2020-10-30 |
| Санкт-Петербург | Булгаков М.А.    | 2020-07-14 |
| Санкт-Петербург | Пастернак Б.Л.   | 2020-05-05 |
| Санкт-Петербург | Лермонтов М.Ю.   | 2020-03-24 |
| Санкт-Петербург | Есенин С.А.      | 2020-01-31 |
+-----------------+------------------+------------+
Affected rows: 15
*/
										
-- 65. Запросы на выборку из нескольких таблиц
/*
...
-- 66. Запросы для нескольких таблиц с группировкой
/*
Посчитать количество экземпляров книг каждого автора на складе. Вывести тех авторов, количество книг которых меньше 10, 
в отсортированном по возрастанию количества виде. Последний столбец назвать Количество.
Пояснение. 
Чтобы в результат были включены авторы, книг которых на складе нет, необходимо в условии отбора, кроме того, 
что общее количество книг каждого автора меньше 10, учесть, что у автора вообще может не быть книг (то есть COUNT(title) = 0).
*/
...
-- 68. Вложенные запросы в операторах соединения
/*
...
WHERE book.genre_id IN (
	SELECT genre_id FROM (
		SELECT genre_id, SUM(amount) AS sum_book
			FROM book
			GROUP BY genre_id
			HAVING sum_book >= MAX(sum_book))
	sum_book)
ORDER BY title;
/*
Query result:
...
-- 69. Операция соединение, использование USING()
/*
...
inner join supply on book.price = supply.price and author.name_author = supply.author;
/*
Query result:
+----------------+-------------+------------+
